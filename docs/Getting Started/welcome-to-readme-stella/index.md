---
title: 欢迎使用 ReadMe
excerpt: 帮助您创建让 API 易于使用和维护的文档。
deprecated: false
hidden: false
metadata:
  title: 欢迎，Stella
  robots: index
---
STELLA ROSE ROSE ROSE

<Cards columns={3}>
  <Card title="快速入门" href="https://docs.readme.com/main/docs/quickstart" icon="fa-rocket-launch">
    了解如何编写和发布文档
  </Card>

  <Card title="API 参考" href="https://docs.readme.com/main/docs/api-reference" icon="fa-code-simple">
    使用 OAS 文件，或通过 ReadMe 创建一个
  </Card>

  <Card title="AI 构建" href="https://docs.readme.com/main/docs/ai-overview#/" icon="fa-sparkles">
    使用 AI 工具编写、审阅和改进文档
  </Card>
</Cards>

<br />

<Anchor label="administration" target="_blank" href="/administration/docs/myjfrog-for-self-hosted">administration</Anchor>

## 最新发布

<Cards columns={3}>
  <Card title="AI 分支审查" href="https://docs.readme.com/main/docs/branches" icon="fa-magnifying-glass">
    在合并前使用 AI Linter 审查分支
  </Card>

  <Card title="与 GitLab 同步" href="https://docs.readme.com/main/docs/sync-with-gitlab" icon="fa-rotate">
    在 GitLab 与 ReadMe 之间双向同步文档
  </Card>

  <Card title="Agent" href="https://docs.readme.com/main/docs/aiagent" icon="fa-message-bot">
    使用 AI 写作助手编辑和审阅您的文档
  </Card>

  <Card title="Linter" href="https://docs.readme.com/main/docs/linter" icon="fa-wand-sparkles">
    使用 AI 强制执行写作风格指南
  </Card>

  <Card title="文档审计" href="https://docs.readme.com/main/docs/docs-audit" icon="fa-brain">
    根据自定义风格指南对您的文档进行评分
  </Card>

  <Card title="MCP 服务器" href="https://docs.readme.com/main/docs/readme-mcp-turbocharge-your-api-for-ai-assistants" icon="fa-robot">
    创建 MCP 服务器
  </Card>

  <Card title="Ask AI" href="https://docs.readme.com/main/docs/ask-ai" icon="fa-user-question">
    让开发者就您的文档提问
  </Card>

  <Card title="组件市场" href="https://docs.readme.com/main/docs/building-custom-mdx-components#marketplace" icon="fa-store">
    发现 MDX 组件的全新方式
  </Card>

  <Card title="在 GitHub 中建议" href="https://docs.readme.com/main/docs/suggest-in-github" icon="fa-file-pen">
    以 GitHub PR 的形式提出更改建议
  </Card>

  <Card title="编辑者角色" href="https://docs.readme.com/main/docs/manage-team#enterprise-roles" icon="fa-user-pen">
    用于编辑内容的团队成员
  </Card>
</Cards>

## ReadMe 基础

<Cards columns={3}>
  <Card title="重构" href="https://docs.readme.com/main/docs/migration-moving-your-docs-to-their-new-home" icon="fa-box-open">
    将您的文档升级到新版 ReadMe
  </Card>

  <Card title="自定义" href="https://docs.readme.com/main/docs/design-themes" icon="fa-brush">
    查看 Logo、品牌和主题选项
  </Card>

  <Card title="集成" href="https://docs.readme.com/main/docs/integrations" icon="fa-arrow-down-left-and-arrow-up-right-to-center">
    与第三方服务集成
  </Card>

  <Card title="示例教程" href="https://docs.readme.com/main/docs/recipes" icon="fa-flask">
    逐步代码演练
  </Card>

  <Card title="更新日志" href="https://docs.readme.com/main/docs/changelog" icon="fa-megaphone">
    让用户了解最新变化
  </Card>

  <Card title="API V2" href="https://docs.readme.com/main/reference/api-migration-guide" icon="fa-brackets-curly">
    使用我们的 API 管理您的文档
  </Card>

  <Card title="CLI" href="https://docs.readme.com/main/docs/upgrading-to-rdme10" icon="fa-terminal">
    使用 rdme 更新您的文档
  </Card>

  <Card title="我的开发者" href="https://docs.readme.com/main/docs/my-developers" icon="fa-user-group-simple">
    在 ReadMe 中获取开发者使用数据
  </Card>

  <Card title="OpenAPI" href="https://docs.readme.com/main/docs/openapi" icon="fa-file-circle-info">
    了解我们对 OAS 的支持
  </Card>

  <Card title="RDMD" href="https://docs.readme.com/rdmd/docs/getting-started" icon="fa-font-case">
    了解 ReadMe 风格的 Markdown
  </Card>

  <Card title="MDX" href="https://docs.readme.com/main/docs/mdx" icon="fa-code">
    编写交互式文档和组件
  </Card>

  <Card title="与 GitHub 同步" href="https://docs.readme.com/main/docs/sync-with-github" icon="fa-code-compare">
    将 ReadMe 连接到 GitHub 项目
  </Card>
</Cards>

## 企业版

<Cards columns={3}>
  <Card title="群组概览" href="https://docs.readme.com/ent/docs/enterprise-group-overview" icon="fa-layer-group">
    管理您的项目和全局设置
  </Card>

  <Card title="用户管理" href="https://docs.readme.com/ent/docs/user-management-overview" icon="fa-user-lock">
    为团队成员和最终用户设置访问权限
  </Card>

  <Card title="预发布环境" href="https://docs.readme.com/ent/docs/publishing-content-with-staging" icon="fa-window-restore">
    内容和设置的预览环境
  </Card>
</Cards>

## 获取支持并保持更新

<Cards columns={2}>
  <Card title="ReadMe 社区" href="https://readme.com/slack" icon="fa-comments">
    获取发布文档的帮助，并与 ReadMe 用户交流。
  </Card>

  <Card title="办公时间" href="https://readme.com/events" icon="fa-handshake">
    观看产品现场演示并向我们的团队提问。
  </Card>
</Cards>

<Cards columns={2}>
  <Card title="支持" href="https://docs.readme.com/main/docs/need-more-support">
    <img src="https://files.readme.io/70d42a5e2dd38b4384f1ba5fcb81572d534e492a70421d6d8013f7b6b0aecbc7-support_1.webp" alt="" />

    联系我们的支持团队或提交新功能请求。
  </Card>

  <Card title="更新日志" href="https://docs.readme.com/main/changelog">
    <img src="https://files.readme.io/2e19c90682aa5e4c722ea2b122c90b9612abf4a489d2d2028da25a18b70bc13c-Changelog_7.webp" alt="" />

    阅读有关新功能、改进和错误修复的内容。
  </Card>
</Cards>